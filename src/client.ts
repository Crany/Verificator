...
export default class ExtendedClient extends Client<true> {
    ...
    private cooldown: Map<string, Date> = new Map<string, Date>(); // User ID, Date object
        ...
        this.on(Events.InteractionCreate, async (interaction) => {
            ...
            try {
                ...
                if (interaction.replied || interaction.deferred) {
                    await interaction.followUp({ content: 'Hmmm, seems like there was an error with that command. The dev team has been notified. <:NymonCross:1135897229660541038>', ephemeral: true });
                } else {
                    await interaction.reply({ content: 'Hmmm, seems like there was an error with that command. The dev team has been notified. <:NymonCross:1135897229660541038>', ephemeral: true });
                }
            }
        });
    }
    ...
    private async register({guildId, clientId, clientToken}: RegisterCommands) {
        ...
        await rest.put(
            ...
            console.log("> There was an error logging registering the commands."); // Red
            console.log("\x1b[0m");
